...
def generate_equation(priv):
    """Генерирует квадратные уравнения только с рациональными корнями."""
    while True:
        if priv:
...
def solve_equation(equation_str):
    """Решает квадратное уравнение и возвращает корни.  Улучшенная версия."""
    try:
...
def parse_user_answer(answer_str):
    """Парсит ответ пользователя с улучшенной обработкой ошибок. Ожидает ввод корней в порядке возрастания."""
    if not answer_str or answer_str.strip() == '-':
        return []

    try:
        # Разделяем по пробелам, преобразуем в числа.  Возвращает None если не все числа целые
        roots = [int(x.strip()) for x in answer_str.split() if x.strip()]
        if len(roots) > 1 and roots[0] > roots[1]:
            return None # Возвращаем None, если корни не в порядке возрастания
        return sorted(roots)
    except ValueError:
        return None
...
def check(equations, user_answers):
    ...
    for i, equation in enumerate(equations):
        ...
        is_correct = user_roots == correct_roots
